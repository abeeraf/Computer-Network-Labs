...
int main() {
	...
	while (1) 
	{
		...
		//Find the maximum subscript for the ready event in the fds array
		for (int x = 0; x < MAXFD; x++) 
		{
			if (fds[x] == -1)
			continue;
			...
			for(int x = 0; x < MAXFD; x++) 
			{
				if (allocatedTickets > 12) 
				{
					...
					string numStr = reservedTickets[x];
						if (numStr.length() > 0) 
						{
							cout <<endl<< "Client " << x << " had tickets: ";
				...
				if (fds[x] == -1)	//If fds[i]==-1, the event is not ready
					continue;
				...
				if (FD_ISSET(fds[x], &fdset)) 
				{
					...
					else 
					{
						...
						{
							index = fds[x];
							if (strncmp(buff, "even", 4) == 0) 
							{
								cout << "This server cannot reserve even tickets." << endl;
							...
							else if (strncmp(buff, "ODD", 3) == 0) 
							{
								cout << endl << "Client " << index << " requesting a ticket." << endl;

								if (allocatedTickets <= 11) 
								{
